"""LLM backend adapters for VCH role agents."""
        ...
        try:
            response = client.responses.create(
                ...
                text={
                    "format": {
                        "type": "json_schema",
                        "name": schema.get("title", "vch_schema"),
                        "schema": schema,
                        "strict": False,
                    }
                },
...
def make_llm_backend(name: str | None, model: str | None = None) -> LLMBackend | None:
    ...
    backend = (name or os.getenv("VCH_LLM_BACKEND") or "deterministic").lower()
    selected_model = model or os.getenv("VCH_LLM_MODEL")
