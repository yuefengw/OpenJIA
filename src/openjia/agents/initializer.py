...
from openjia.schemas.run_state import RunState


class Initializer:
    """
    Initializes the OpenJIA harness and environment.
    ...
    def _create_run_state(self, user_task: str) -> RunState:
        ...
        run_id = f"{datetime.now().strftime('%Y-%m-%dT%H-%M-%S')}-openjia"
