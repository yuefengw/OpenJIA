"""OpenJIA CLI - Command line interface for OpenJIA harness."""
...
def main():
    """OpenJIA agent harness for verifiable long-running software development."""
    pass
...
def init(repo: str, max_sprints: int, max_repair_attempts: int):
    """Initialize OpenJIA harness in a repository."""
    ...
    click.echo(f"Initialized OpenJIA harness in {repo_path}")
...
def plan(
    ...
    if not harness_dir.exists():
        click.echo("Error: OpenJIA not initialized. Run 'openjia init' first.", err=True)
        sys.exit(1)
...
def run(
    ...
    """Run OpenJIA harness for a task."""
    repo_path = Path(repo).absolute()
...
def run_sprint(sprint_id: str, repo: str):
    ...
    click.echo(
        ...
        "use 'openjia run' for the current deterministic MVP."
    )
    sys.exit(2)
...
def llm_smoke(llm_backend: str, model: str):
    ...
    from openjia.llm import LLMConfigurationError, make_llm_backend
    ...
    try:
        ...
        result = backend.generate_json(
            ...
            prompt="Return a JSON object with ok=true and provider='openjia'.",
            schema={
                "title": "openjia_smoke_test",
                "type": "object",
                "properties": {
                    "ok": {"type": "boolean"},
...
def repair(sprint_id: str, repo: str):
    ...
    click.echo("Use 'openjia run' to restart the full harness.")
