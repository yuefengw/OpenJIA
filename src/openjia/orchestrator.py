"""HarnessOrchestrator - main state machine for OpenJIA."""
...
from openjia.gates.evaluation_gate import EvaluationGate, EvaluationDecision, DecisionKind
...
import yaml
from openjia.feature_ledger import load_ledger, save_ledger, update_ledger_from_eval, write_progress_markdown


class HarnessOrchestrator:
    """
    Main orchestrator for the OpenJIA harness.
    ...
    def _commit_sprint(self, sprint, harness_dir: Path) -> None:
        ...
        try:
            ...
            subprocess.run(
                ["git", "commit", "-m", f"OpenJIA: Complete sprint {sprint.id}"],
                cwd=self.repo_root,
                capture_output=True
            )
