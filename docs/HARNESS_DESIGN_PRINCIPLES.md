...
This document records the source principles that should guide OpenJIA changes. Future implementation work should update the harness in alignment with these constraints rather than adding agent complexity for its own sake.
...
DeepAgents or any LLM runtime is the execution substrate. OpenJIA is the harness protocol: artifacts, gates, evidence, state transitions, and repair loops.
